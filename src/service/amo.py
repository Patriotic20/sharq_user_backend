...
class FieldBuilder:    
    @staticmethod
    def create_field_value(field_id: str, value: Any, use_field_code: bool = False) -> Dict[str, Any]:
    ...
    def build_fields_from_mappings(
        mappings: List[Tuple[str, Optional[str], Any]], 
        ...
        for field_name, field_id, value in mappings:
            ...
            fields.append(FieldBuilder.create_field_value(field_id, value, use_field_code))
...
class AmoCRMService:
    ...
    def _handle_request_error(self, error: Exception, method: str, endpoint: str) -> None:
    ...
    def _get_embedded_data(self, data: Dict[str, Any], entity_type: str) -> List[Dict[str, Any]]:
        return data.get("_embedded", {}).get(entity_type, [])

    def _get_first_entity(self, data: Dict[str, Any], entity_type: str) -> Optional[Dict[str, Any]]:
        entities = self._get_embedded_data(data, entity_type)
        return entities[0] if entities else None

    def get_status_by_id(self, pipeline_id: int, status_id: int) -> Optional[Dict[str, Any]]:
        try:
            ...
            if not status:
                logger.warning(f"Status ID {status_id} not found in pipeline {pipeline_id}")
                return None
                
            return status
            
        except AmoCRMException:
            logger.error(f"Failed to get status for pipeline {pipeline_id}, status {status_id}")
            return None
    ...
    def _get_contact_fields(self) -> Dict[str, int]:
        return self._get_cached_fields("contacts/custom_fields", "_contact_fields_cache")

    def _get_lead_fields(self) -> Dict[str, int]:
        return self._get_cached_fields("leads/custom_fields", "_lead_fields_cache")

    def _build_contact_fields_values(self, contact_data: ContactData) -> List[Dict[str, Any]]:
        ...
        field_mappings = [
            ...
            ("familya", fields.get(CONTACT_FIELD_MAPPINGS["familya"]), contact_data.last_name),
            ("ota ismi", fields.get(CONTACT_FIELD_MAPPINGS["ota ismi"]), contact_data.middle_name),
            ("должность", fields.get(CONTACT_FIELD_MAPPINGS["должность"]), contact_data.position),
            ("tug'ilgan kuni", fields.get(CONTACT_FIELD_MAPPINGS["tug'ilgan kuni"]), contact_data.birthdate),
            ("jinsi", fields.get(CONTACT_FIELD_MAPPINGS["jinsi"]), 
             contact_data.gender if contact_data.gender else None),
            ("country", fields.get(CONTACT_FIELD_MAPPINGS["country"]), contact_data.country),
            ("region", fields.get(CONTACT_FIELD_MAPPINGS["region"]), contact_data.region),
            ("district", fields.get(CONTACT_FIELD_MAPPINGS["district"]), contact_data.district),
            ("manzil", fields.get(CONTACT_FIELD_MAPPINGS["manzil"]), contact_data.address),
        ]
        
        return FieldBuilder.build_fields_from_mappings(field_mappings, CONTACT_FIELD_MAPPINGS)
    ...
    def _build_deal_fields_values(self, deal_data: DealData) -> List[Dict[str, Any]]:
        ...
        field_mappings = [
            ("talim tili", fields.get(DEAL_FIELD_MAPPINGS["talim tili"]), deal_data.edu_lang_id),
            ("talim turi", fields.get(DEAL_FIELD_MAPPINGS["talim turi"]), deal_data.edu_type),
            ("talim shakli", fields.get(DEAL_FIELD_MAPPINGS["talim shakli"]), deal_data.edu_form),
            ("talim yo'nalishi", fields.get(DEAL_FIELD_MAPPINGS["talim yo'nalishi"]), deal_data.edu_direction),
            ("o'rta talim tugatgan yili", fields.get(DEAL_FIELD_MAPPINGS["o'rta talim tugatgan yili"]), 
             self._format_to_amocrm_date(deal_data.edu_end_date)),
            ("admission id", fields.get(DEAL_FIELD_MAPPINGS["admission id"]), str(deal_data.admission_id)),
            ("certificate fayl", fields.get(DEAL_FIELD_MAPPINGS["certificate fayl"]), deal_data.certificate_link),
            ("pasport fayl", fields.get(DEAL_FIELD_MAPPINGS["pasport fayl"]), deal_data.passport_file_link),
        ]
        
        return FieldBuilder.build_fields_from_mappings(field_mappings, DEAL_FIELD_MAPPINGS)
    ...
    def _create_deal_with_pipeline(self, deal_data: DealData, pipeline_type: str, tags: List[str]) -> Optional[Dict[str, Any]]:
    ...
    def create_deal(self, deal_data: DealData) -> Optional[Dict[str, Any]]:
        try:
            return self._create_deal_with_pipeline(deal_data, PIPELINE_TYPES["FIRST_CREATE"], ["Qabul sayt"])
        except AmoCRMException as e:
    ...
    def create_initial_contact_with_phone(self, phone: str) -> Optional[Dict[str, Any]]:
        try:
            ...
            logger.error(f"Failed to create initial contact/deal with phone {phone}: {e}")
            return None
    ...
    def update_contact_with_full_data(
        self, 
        deal_id: int, 
        deal_data: DealData
    ) -> bool:
...
def create_initial_lead(phone: str, config_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    try:
...
def update_lead_with_passport_data(
    contact_id: int,
    passport_data: PassportDataBase,
