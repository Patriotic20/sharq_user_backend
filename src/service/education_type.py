...
from sharq_models.models import EducationType
...
class EducationTypeCrud(BasicCrud[EducationType, EducationTypeBase]):
    ...
    async def create_education_type(self, obj: EducationTypeBase) -> EducationTypeResponse:
    ...
    async def get_by_education_type_id(self, form_id: int) -> EducationTypeResponse:
        education_type = await super().get_by_id(model=EducationType, item_id=form_id)
    ...
    async def update_education_type(
        self, form_id: int, obj: EducationTypeUpdate
    ) -> EducationTypeResponse:
        await self.get_by_education_type_id(form_id)
        return await super().update(model=EducationType, item_id=form_id, obj_items=obj)

    async def delete_education_type(self, form_id: int) -> dict:
        await self.get_by_education_type_id(form_id)
        return await super().delete(model=EducationType, item_id=form_id)
